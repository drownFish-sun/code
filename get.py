...
from utils.template_split import process_logs
...
def load_data(file, dataset, window_size=120):
    ...
    logs = pd.read_csv(os.path.join(PROJECT_ROOT, os.pardir, 'dataset', dataset, dataset + '.log_structured.csv'))
    seq_labels = []
    seq_idxs = []
    # tj = np.zeros(5)
    if dataset == 'hades':
        pbar = tqdm(total=len(logs))
        l_ = logs['Label'].to_numpy()
        lineId = logs['LineId'].to_numpy()
        for i in range(0, len(logs), window_size):
            l_chunk = l_[i:i + window_size]
            id_chunk = list(lineId[i:i + window_size])

            seq_label = int(np.any(l_chunk != '-'))  # 利用 NumPy 加速 any 判断
            # y_ = (l_chunk != '-').astype(int)
            # if np.sum(y_) / len(y_) < 0.2:
            #     tj[0] += 1
            # elif np.sum(y_) / len(y_) < 0.4:
            #     tj[1] += 1
            # elif np.sum(y_) / len(y_) < 0.6:
            #     tj[2] += 1
            # elif np.sum(y_) / len(y_) < 0.8:
            #     tj[3] += 1
            # else:
            #     tj[4] += 1
        ...
        for _n, group in grouped:
            ...
            for i in range(0, len(lineId), window_size):
                ...
                seq_idxs.append(id_chunk)
            pbar.update(1)
    # tj = tjj / np.sum(tj) * 100
    # print(tj)
    seq_labels = np.array(seq_labels)
    N = len(seq_labels)
    print(N)
    idx = np.random.permutation(N)
    seq_labels = seq_labels[idx]
    seq_idxs = [seq_idxs[i] for i in idx]
    N_train = int(N * 0.6)
    N_val = int(N * 0.1)
    train_seq = seq_idxs[:N_train]
    val_seq = seq_idxs[N_train:N_val+N_train]
    test_seq = seq_idxs[N_val+N_train:]
    seq_labels = seq_labels[N_val+N_train:]
    train_idxs = []
    for seq in train_seq:
        train_idxs.extend([id for id in seq])
    val_idxs = []
    for seq in val_seq:
        val_idxs.extend([id for id in seq])
    test_idxs = []
    for seq in test_seq:
        test_idxs.extend([id for id in seq])
    new_row = pd.DataFrame([{}], columns=logs.columns)  # 空行
    logs = pd.concat([new_row, logs], ignore_index=True)
    train_logs = logs.iloc[train_idxs]
    val_logs = logs.iloc[val_idxs]
    test_logs = logs.iloc[test_idxs]
    label_idxs = np.array(test_logs['LineId'])
    train_nodes = train_logs['EventTemplate'].unique()
    test_nodes = test_logs['EventTemplate'].unique()
    val_nodes = val_logs['EventTemplate'].unique()

    train_logs_labels = train_logs['Label'].to_numpy()
    train_logs_labels = np.where(train_logs_labels == '-', 0, 1)
    val_logs_labels = val_logs['Label'].to_numpy()
    val_logs_labels = np.where(val_logs_labels == '-', 0, 1)
    test_logs_labels = test_logs['Label'].to_numpy()
    test_logs_labels = np.where(test_logs_labels == '-', 0, 1)
    train_nodes = np.array(train_nodes)
    test_nodes = np.array(test_nodes)
    val_nodes = np.array(val_nodes)

    train_nodes, train_labels, replace_map = process_logs(train_logs, train_nodes, None)
    val_nodes, val_labels, _ = process_logs(val_logs, val_nodes, replace_map)
    test_nodes, test_labels, _ = process_logs(test_logs, test_nodes, replace_map)
    # replace = {}
    # extra_labels = []
    # extra_nodes = []
    ...
    #     l_ = val_logs[val_logs['EventTemplate'] == node]['Label']
    #     y_ = 0
    #     for _ in l_:
    #         y_ += 0 if _ == '-' else 1
    ...
    #     else:
    #         # print(node)
    #         para_ = val_logs[val_logs['EventTemplate'] == node]['ParameterList'].drop_duplicates()
    #         para_ = list(para_)
    #         if replace[node] == -1:
    #             level_ = val_logs[val_logs['EventTemplate'] == node]['Level'].drop_duplicates()
    #             level_ = list(level_)
    #             for j, level in enumerate(level_):
    #                 node_ = node + ' ' + level
    #                 indexer = (val_logs['EventTemplate'] == node) & (val_logs['Level'] == level)
    #                 _y = list(val_logs[(val_logs['EventTemplate'] == node) & (val_logs['Level'] == level)][
    #                               'Label'].drop_duplicates())
    #                 val_logs.loc[indexer, 'EventTemplate'] = node_
    #                 # print(_y)
    #                 _ = 0
    #                 if len(_y) == 1:
    ...
    #             rep = actual_list[len(actual_list) - k - 1] if k < len(actual_list) else '<*>'
    #             node_ = replace_nth(node_, u_, v_, rep)
    #             _y = list(val_logs[(val_logs['EventTemplate'] == node) & (val_logs['ParameterList'] == para)][
    #                           'Label'].drop_duplicates())
    #             indexer = (val_logs['EventTemplate'] == node) & (val_logs['ParameterList'] == para)
    #             val_logs.loc[indexer, 'EventTemplate'] = node_
    #             # print(_y)
    #             _ = 0
    #             if len(_y) == 1:
    ...
    #         extra_labels_.append(extra_labels[i])
    #
    # val_nodes = np.array(extra_nodes_)
    # val_labels = np.array(extra_labels_, dtype=int)
    #
    # extra_nodes = []
    # extra_labels = []
    ...
    id_dev = {}
    for i, node in enumerate(val_nodes):
        id_dev[i] = np.where(val_logs['EventTemplate'] == node)[0]
    id_test = {}
    for i, node in enumerate(test_nodes):
        id_test[i] = np.where(test_logs['EventTemplate'] == node)[0]
    return np.concatenate([train_nodes, val_nodes, test_nodes]), \
        np.concatenate([train_labels, val_labels, test_labels]), \
        train_logs_labels, test_logs_labels, val_logs_labels, test_nodes, \
        len(train_nodes), len(train_nodes) + len(val_nodes), None, id_train, id_test, id_dev, \
        seq_labels, test_seq, label_idxs