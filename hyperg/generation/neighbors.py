...
def gen_knn_hg(X, n_neighbors, is_prob=True, with_feature=False):
    ...
    edge_idx = np.tile(np.arange(n_nodes).reshape(-1, 1), (1, n_neighbors+1)).reshape(-1)
    ...
    H = sparse.coo_matrix((values, (node_idx, edge_idx)), shape=(n_nodes, n_nodes))

    dist = np.zeros((n_nodes, n_nodes))
    for i in range(n_nodes):
        for j in range(i + 1, n_nodes):
            dist[i, j] = dist[j, i] = sum(abs(a - b) for a, b in zip(X[i], X[j]))
    m_neighbors = np.argpartition(dist, kth=n_neighbors + 1, axis=1)
    m_neighbors_val = np.take_along_axis(dist, m_neighbors, axis=1)
    ...
    H2 = sparse.coo_matrix((values, (node_idx, edge_idx)), shape=(n_nodes, n_nodes))
    H = np.hstack((H.toarray(), H2.toarray()))
